...
N = 30000 # memory for storing parameters 
episodes = 20
...
def main():
    ...
    x = range(episodes)   
    plt.title('CartPole-v1: Expectedsarsa')
...
def episode(episode_num, i_episode_num, epsilon, theta, max_steps):
    ...
    while True:
        ...
        if not done:
            Vpi = sum([Q[action] * ((action == a) * (1.-epsilon) + epsilon * (1./len(actionspace))) for a in actionspace])
            delta = Q[action] + alpha * (reward + gamma * Vpi - Q[action])
