...
for episode in range(episodes):
    for i_episode in range(episodes):
        ...
        while True:
            ...
            Vpi = sum([q_table[pos][vel][a] * ((act == a) * (1.-epsilon) + epsilon * (1./len(actionspace))) for a in actionspace])

            q_table[pos][vel][act] = q_table[pos][vel][act] + alpha *(reward + gamma * Vpi - q_table[pos][vel][act])
            steps += 1
            if done :
                # env.close()
                break
...
plt.title('MountiainCar: Expected Sarsa')
